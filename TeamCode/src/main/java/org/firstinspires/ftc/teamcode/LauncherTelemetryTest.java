...
@TeleOp(name = "Kurry TeleOp V1", group = "Linear Opmode")
public class LauncherTelemetryTest extends LinearOpMode {
    ...
    // Encoder constants for RPM calculation
    private static final double LAUNCHER_TICKS_PER_REV = 28;
    private int lastLauncherLeftTicks = 0;
    private int lastLauncherRightTicks = 0;
    ...
    public void runOpMode() {
        ...
        // Initialize last encoder readings for RPM calculation
        lastLauncherLeftTicks = launcherLeft.getCurrentPosition();
        lastLauncherRightTicks = launcherRight.getCurrentPosition();
        ...
        while (opModeIsActive()) {
            // --- Drive control (mecanum/tank hybrid) ---
            ...
            // --- Flapper control ---
            if (gamepad2.b) flapperLeft.setPosition(0.14);
            else flapperLeft.setPosition(flapperLeftPosition);

            if (gamepad2.dpad_left) flapperRight.setPosition(0.58);
            else flapperRight.setPosition(flapperRightPosition);
            ...
            int currentLeftTicks = launcherLeft.getCurrentPosition();
            int currentRightTicks = launcherRight.getCurrentPosition();
            long currentTime = System.nanoTime();

            int deltaLeft = currentLeftTicks - lastLauncherLeftTicks;
            int deltaRight = currentRightTicks - lastLauncherRightTicks;
            double deltaTimeSec = (currentTime - lastTimeNano) / 1e9; // ns → sec

            double leftRPM = (deltaLeft / LAUNCHER_TICKS_PER_REV) / deltaTimeSec * 60.0;
            double rightRPM = (deltaRight / LAUNCHER_TICKS_PER_REV) / deltaTimeSec * 60.0;

            lastLauncherLeftTicks = currentLeftTicks;
            lastLauncherRightTicks = currentRightTicks;
            lastTimeNano = currentTime;

            // --- Telemetry ---
            telemetry.addData("Left Launch Power", "%.2f", powerL);
            telemetry.addData("Right Launch Power", "%.2f", powerR);
            telemetry.addData("Left Launcher RPM", "%.1f", leftRPM);
            telemetry.addData("Right Launcher RPM", "%.1f", rightRPM);
