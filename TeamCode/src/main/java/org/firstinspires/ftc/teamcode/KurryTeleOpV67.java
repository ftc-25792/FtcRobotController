...
import org.firstinspires.ftc.teamcode.AprilTagHelper;
import org.firstinspires.ftc.vision.apriltag.AprilTagDetection;

import java.util.List;

@TeleOp(name = "--KURRYTeleOpV.M&K", group = "Linear Opmode")
public class KurryTeleOpV67 extends LinearOpMode {
    ...
    enum DriveControl { Mithil, Hanish}

    private DriveState state = DriveState.DRIVE;
    private Alliance alliance = Alliance.RED;
    private DriveControl driveControl = DriveControl.Hanish;

    private double allianceSign = -1;
    private double driveControlSign = -1;
    ...
    public void runOpMode() {
        ...
        while (!isStarted()) {
            if (gamepad1.a) {
                ...
                driveControl = DriveControl.Hanish;
            } else if (gamepad1.y) {
                alliance = Alliance.BLUE;
                driveControl = DriveControl.Hanish;
            } else if (gamepad1.x) {
                alliance = Alliance.RED;
                driveControl = DriveControl.Mithil;
            } else if (gamepad1.b) {
                alliance = Alliance.BLUE;
                driveControl = DriveControl.Mithil;
            }
            allianceSign = (alliance == Alliance.BLUE) ? 1 : -1;
            driveControlSign = (driveControl == DriveControl.Hanish) ? 1 : -1;
            telemetry.addData("Alliance: ", alliance);
            telemetry.addData("Drive Control: ", driveControl);
            telemetry.update();
        }
    ...
    private void alignToPost() {
        ...
        double drive  = Range.clip(rangeError * SPEED_GAIN,
                -MAX_DRIVE, MAX_DRIVE);
        double strafe = Range.clip(-bearingError * STRAFE_GAIN,
    ...
    private AprilTagDetection getClosestTag(List<AprilTagDetection>
                                                    detections) {
    ...
    private double launcherPID(double target, double current, double
            dt, boolean isLeft) {
